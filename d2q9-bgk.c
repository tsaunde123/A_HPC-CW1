...
int propagate_mid(int params_nx, int params_ny, float params_omega, float* cells, float* tmp_cells, float* halo_cells, int local_ncols, int local_nrows, int nlr_nrows,
              ...
              float* tmp_halo_topline, float* tmp_halo_bottomline)
{
 ...
 //{}
  ...
  for (int ii = 0; ii < local_ncols; ii++){
    ...
    // } else{ //COLLISION
    /* compute local density total */
    float local_density = local0 + local1 + local2 + local3 + local4 + local5 + local6 + local7 + local8;    /* compute x velocity component */
    /* compute x velocity component */
    float u_x = (local1
                  + local5
                  + local8
