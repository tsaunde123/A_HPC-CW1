...
int timestep(const t_param params, float* cells, float* tmp_cells, int* obstacles, float* halo_cells, int* halo_obs, int local_nrows, int local_ncols, int size, int rank, int halo_local_nrows, int halo_local_ncols, int nlr_nrows, float* halo_temp, MPI_Status status, int top,
 int bottom, MPI_Datatype MPI_cell_type, MPI_Request request);
int accelerate_flow(const t_param params, float* cells, int* obstacles, float* halo_cells, int* halo_obs, int local_nrows);
...
int main(int argc, char* argv[])
{
  ...
  if(rank == MASTER){
    ...
    if(rank < rest){
      ...
      MPI_Recv(recvcbuf, 9*extra_local_nrows*local_ncols, MPI_FLOAT, MASTER, tag, MPI_COMM_WORLD, &status);
      for(int speed = 0; speed < NSPEEDS; speed++){
  ...
  {
    timestep(params, cells, tmp_cells, obstacles, halo_cells, halo_obs, local_nrows, local_ncols, size, rank, halo_local_nrows, halo_local_ncols, nlr_nrows, halo_temp, status, top, bottom, MPI_cell_type, request);
  ...
  if(rank != MASTER){
    if(rank < rest){
      ...
      MPI_Send(sendcbuf, 9*extra_local_nrows*local_ncols, MPI_FLOAT, MASTER, tag, MPI_COMM_WORLD);
    } else {
      for(int speed = 0; speed < NSPEEDS; speed++){
        for(int jj = 0; jj < local_nrows * local_ncols; jj++){
          sendcbuf[(local_ncols*local_nrows) * speed + jj] = halo_cells[(local_ncols*(local_nrows+2)) * speed + jj + halo_local_ncols*1];
...
int accelerate_flow(const t_param params, float* cells, int* obstacles, float* halo_cells, int* halo_obs, int local_nrows)
{
  ...
  for (int ii = 0; ii < params.nx; ii++)
  {
    ...
    if (!halo_obs[ii + o_jj*params.nx]
        && (halo_cells[(local_ncols*(local_nrows+2)) * 3 + ii + h_jj*params.nx] - w1) > 0.f
        && (halo_cells[(local_ncols*(local_nrows+2)) * 6 + ii + h_jj*params.nx] - w2) > 0.f
        && (halo_cells[(local_ncols*(local_nrows+2)) * 7 + ii + h_jj*params.nx] - w2) > 0.f)
    {
      // increase 'east-side' densities
      halo_cells[(local_ncols*(local_nrows+2)) * 1 + ii + h_jj*params.nx] += w1;
      halo_cells[(local_ncols*(local_nrows+2)) * 5 + ii + h_jj*params.nx] += w2;
      halo_cells[(local_ncols*(local_nrows+2)) * 8 + ii + h_jj*params.nx] += w2;
      // decrease 'west-side' densities
      halo_cells[(local_ncols*(local_nrows+2)) * 3 + ii + h_jj*params.nx] -= w1;
      halo_cells[(local_ncols*(local_nrows+2)) * 6 + ii + h_jj*params.nx] -= w2;
      halo_cells[(local_ncols*(local_nrows+2)) * 7 + ii + h_jj*params.nx] -= w2;
    }
  }
              ...
              MPI_Datatype MPI_cell_type, int top, int bottom, int* halo_obs)
{
  ...
  sendcbuf[local_ncols * speed + jj] = halo_cells[(local_ncols*(local_nrows+2)) * speed + jj + halo_local_ncols*1];
  ...
  for (int jj = 1; jj < local_nrows-1; jj++){
    for (int ii = 0; ii < local_ncols; ii++){
      ...
      halo_temp[(local_ncols*(local_nrows+2)) * 0 + (ii + (jj+1)*params.nx)] = halo_cells[(local_ncols*(local_nrows+2)) * 0 + ii + (jj+1)*halo_local_ncols]; /* central cell, no movement */
      halo_temp[(local_ncols*(local_nrows+2)) * 1 + (ii + (jj+1)*params.nx)] = halo_cells[(local_ncols*(local_nrows+2)) * 1 + x_w + (jj+1)*local_ncols]; /* east */
      halo_temp[(local_ncols*(local_nrows+2)) * 2 + (ii + (jj+1)*params.nx)] = halo_cells[(local_ncols*(local_nrows+2)) * 2 + ii + y_s*local_ncols]; /* north */
      halo_temp[(local_ncols*(local_nrows+2)) * 3 + (ii + (jj+1)*params.nx)] = halo_cells[(local_ncols*(local_nrows+2)) * 3 + x_e + (jj+1)*local_ncols]; /* west */
      halo_temp[(local_ncols*(local_nrows+2)) * 4 + (ii + (jj+1)*params.nx)] = halo_cells[(local_ncols*(local_nrows+2)) * 4 + ii + y_n*local_ncols]; /* south */
      halo_temp[(local_ncols*(local_nrows+2)) * 5 + (ii + (jj+1)*params.nx)] = halo_cells[(local_ncols*(local_nrows+2)) * 5 + x_w + y_s*local_ncols]; /* north-east */
      halo_temp[(local_ncols*(local_nrows+2)) * 6 + (ii + (jj+1)*params.nx)] = halo_cells[(local_ncols*(local_nrows+2)) * 6 + x_e + y_s*local_ncols]; /* north-west */
      halo_temp[(local_ncols*(local_nrows+2)) * 7 + (ii + (jj+1)*params.nx)] = halo_cells[(local_ncols*(local_nrows+2)) * 7 + x_e + y_n*local_ncols]; /* south-west */
      halo_temp[(local_ncols*(local_nrows+2)) * 8 + (ii + (jj+1)*params.nx)] = halo_cells[(local_ncols*(local_nrows+2)) * 8 + x_w + y_n*local_ncols]; /* south-east */
    }
  }
  for (int jj = 1; jj < local_nrows-1; jj++){
    for (int ii = 0; ii < local_ncols; ii++){
  ...
  for (int jj = 1; jj < local_nrows-1; jj++){
    for (int ii = 0; ii < local_ncols; ii++){
      if (!halo_obs[ii + jj*params.nx]){ //COLLISION
        ...
        float u_x = (halo_temp[(local_ncols*(local_nrows+2)) * 1 + ii + (jj+1)*params.nx]
                      + halo_temp[(local_ncols*(local_nrows+2)) * 5 + ii + (jj+1)*params.nx]
                      + halo_temp[(local_ncols*(local_nrows+2)) * 8 + ii + (jj+1)*params.nx]
                      - (halo_temp[(local_ncols*(local_nrows+2)) * 3 + ii + (jj+1)*params.nx]
                         + halo_temp[(local_ncols*(local_nrows+2)) * 6 + ii + (jj+1)*params.nx]
                         + halo_temp[(local_ncols*(local_nrows+2)) * 7 + ii + (jj+1)*params.nx]))
                     / local_density;
        /* compute y velocity component */
        float u_y = (halo_temp[(local_ncols*(local_nrows+2)) * 2 + ii + (jj+1)*params.nx]
                      + halo_temp[(local_ncols*(local_nrows+2)) * 5 + ii + (jj+1)*params.nx]
                      + halo_temp[(local_ncols*(local_nrows+2)) * 6 + ii + (jj+1)*params.nx]
                      - (halo_temp[(local_ncols*(local_nrows+2)) * 4 + ii + (jj+1)*params.nx]
                         + halo_temp[(local_ncols*(local_nrows+2)) * 7 + ii + (jj+1)*params.nx]
                         + halo_temp[(local_ncols*(local_nrows+2)) * 8 + ii + (jj+1)*params.nx]))
                     / local_density;
        /* velocity squared */
        ...
        /* relaxation step */
        for (int kk = 0; kk < NSPEEDS; kk++){
          halo_temp[(local_ncols*(local_nrows+2)) * kk + ii + (jj+1)*params.nx] = halo_temp[(local_ncols*(local_nrows+2)) * kk + ii + (jj+1)*params.nx]
                                                  + params.omega
                                                  * (d_equ[kk] - halo_temp[(local_ncols*(local_nrows+2)) * kk + ii + (jj+1)*params.nx]);
        }
      }
    }
  }
